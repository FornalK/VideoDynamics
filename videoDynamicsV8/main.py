        ...
        if len(tabOfScenesFirstFramesIndexes) > 1:
            if int(pos_frame) == (tabOfScenesFirstFramesIndexes[scene_num + 1]):
                ...
                if amount == 1:
                    ...
                    sampledScene = []
                    randomFramesIndexes = RandomNumberGenerator.getRandomNonRepeatingIntegers(0, len(frames), amount)
...
def main(videoPath, sampling = None, interval = None, tabOfScenesFirstFramesIndexes = [], shift = 0.1):
    ...
    if tabOfScenesFirstFramesIndexes != []:
        ...
        dataPackage['mean internal scenes distance (chinese)'] = internalScenes_d
        dataPackage['distance between individual scenes (chinese)'] = externalScenes_d
        dataPackage['mean internal scenes distance (Agrawal)'] = internalScenes_d2
        dataPackage['distance between individual scenes (Agrawal)'] = externalScenes_d2
...
def WholeFilm(videoPath, sampling = 10, repetitions = 1):
    if sampling <= 0 or sampling > 100:
        print("Error: sampling must be between (0, 100]")
        raise Exception
    else:
        vidcap = cv2.VideoCapture(videoPath)
        frames = []
        ...
        while True:
            ...
            if vidcap.get(cv2.CAP_PROP_POS_FRAMES) == frame_count:
                break
            if pos_frame % 100 == 0:
                print(pos_frame)
        ...
        for i in range(repetitions):
            ...
            randomFramesIndexes = RandomNumberGenerator.getRandomNonRepeatingIntegers(0, len(frames), amount)
            for index in randomFramesIndexes:
                sampledVideo.append(frames[index].copy())

            # chinese measure
            selectedFrames = toGrayscaleAndNormalize(sampledVideo)
            internalScenes_d = measureDistance(selectedFrames)

            # Prateek Agrawal measure
            internalScenes_d2 = agrawalMethod(sampledVideo)

            print("mean distance (chinese): ", internalScenes_d)
            print("mean distance (Agrwal): ", internalScenes_d2)

            chineseResults.append(internalScenes_d)
            agrawalResults.append(internalScenes_d2)

        meanChineseResult = np.mean(chineseResults)
        meanAgrawalResult = np.mean(agrawalResults)

        print("final distance (chinese): ", meanChineseResult)
        print("final distance (Agrwal): ", meanAgrawalResult)

        numberOfFramesTaken = len(sampledVideo)
        ...
        dataPackage['sample size (%)'] = sampling
        dataPackage['number of frames taken'] = numberOfFramesTaken
        dataPackage['mean internal scenes distance (chinese)'] = meanChineseResult
        dataPackage['distance between individual scenes (chinese)'] = meanAgrawalResult
...
dataPackage = WholeFilm(videoPath, sampling=10, repetitions=10)
